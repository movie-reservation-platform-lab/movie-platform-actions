...
export function parseEvaluatorOutput(outputText) {
...
export function validateStrictResult(output) {
...
function requireDiagnostic(condition, explanation) {
    if (!condition)
        throw new Error(`Report/evaluator failure: ${explanation}.`);
}
/**
 ...
 */
export function validateApprovedDiagnostic(diagnostic, expectedImage, output) {
    ...
    requireDiagnostic(evaluation.counts.high === Number(output.highCount) && evaluation.counts.critical === Number(output.criticalCount), "diagnostic and subprocess raw finding counts disagree");
    ...
    requireDiagnostic(isFindingCount(blockingCritical) && isFindingCount(notAffectedCritical) && isFindingCount(riskAcceptedCritical), "blocking and exempted finding counts must be nonnegative safe integers");
    requireDiagnostic(blockingCritical + notAffectedCritical + riskAcceptedCritical === evaluation.counts.critical, "blocking and exempted counts do not account for every raw CRITICAL");
    requireDiagnostic((blockingCritical === 0) === (output.expectedExitCode === 0), "blocking CRITICAL count disagrees with the policy exit code");
    requireDiagnostic((evaluation.result === "passed") === (evaluation.counts.critical === 0), "a pass with raw CRITICAL findings must be identified as passed-with-exemptions");
}

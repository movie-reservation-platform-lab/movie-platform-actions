...
export async function runTrivy(endpoint, socket, image, name, env) {
    const child = spawn("docker", ["--host", endpoint, ...scannerArguments(socket, image, name)], { env, stdio: ["ignore", "pipe", "pipe"] });
...
function collectOutput(child) {
    return new Promise((resolve) => {
        ...
        const collectReportChunk = (chunk) => {
...
function registerCancellation(stop) {
...
function classifyFailure(diagnostics) {
    return /database|vulnerability DB|java DB|db error|db update/i.test(diagnostics.toString("utf8"))
...
function removeScannerContainer(endpoint, name, env) {
    const cleanup = spawnSync("docker", ["--host", endpoint, "rm", "--force", name], { env, stdio: "ignore", timeout: 10_000, killSignal: "SIGKILL" });
    if (cleanup.error) {

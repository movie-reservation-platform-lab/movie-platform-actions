...
export function scannerArguments(socket: string, image: string, name: string): string[] {
  return [
    "run", "--rm", "--name", name,
...
export async function scan(
  endpoint: string, socket: string, image: string, name: string,
  env: NodeJS.ProcessEnv,
): Promise<{ report: Buffer; failure: string | undefined }> {
  const result = await new Promise<{ report: Buffer; failure: string | undefined }>((resolve) => {
    ...
    child.stdout.on("data", (chunk: Buffer) => {
      ...
      if (retained.length !== chunk.length) stop("Scanner report exceeded the 64 MiB limit.");
    });

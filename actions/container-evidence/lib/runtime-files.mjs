...
export function parseJson(bytes) {
    ...
    const skipWhitespace = () => {
        while (position < jsonText.length && /\s/.test(jsonText[position]))
            position++;
    };
    ...
    const readJsonString = () => {
        ...
        while (position < jsonText.length) {
            ...
            if (character === "\\")
                ...
                return JSON.parse(jsonText.slice(stringStart, position));
        }
        throw new Error();
    };
    const checkJsonValue = (depth) => {
    ...
    try {
        checkJsonValue(0);
        skipWhitespace();
        if (position !== jsonText.length)
            throw new Error();
        return JSON.parse(jsonText, (_key, parsedValue) => {

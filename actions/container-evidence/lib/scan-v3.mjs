...
export function hostedScannerArguments(image, format, name, registryConfig) {
...
function docker(args, env, timeout, limit) {
...
export function scanHosted(env, run = docker) {
    ...
    const scannerEnv = { PATH: env.PATH, DOCKER_CONFIG: config };
    try {
        ...
        for (const [format, filename] of [["cyclonedx", profile.sbom], ["json", profile.vulnerabilities]]) {
            ...
            try {
                ...
                try {
                    run(["--host", "unix:///var/run/docker.sock", "rm", "--force", name], scannerEnv, 10_000, 64 * 1024);
                }
                catch { /* bounded cleanup only */ }

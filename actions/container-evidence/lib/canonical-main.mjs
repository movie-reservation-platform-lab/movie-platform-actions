...
function canonicalCommitSha(bytes) {
    try {
        const reference = parseJson(bytes);
        if (isObject(reference) && reference.ref === "refs/heads/main" &&
...
export async function resolveCanonicalMain(component, token) {
            ...
            clientRequest = request({
                ...
                incoming.on("close", () => {
                    if (!incoming.complete)
                        fail("canonical-main-unavailable");
                });
                if (settled) {
                    incoming.destroy();
                    return;
                ...
                incoming.on("data", (chunk) => {
                    ...
                    if (receivedBytes > maxResponseBytes)
                        fail("canonical-main-response-too-large");
                    else
                        chunks.push(chunk);
                });
                incoming.on("end", () => {
    ...
    if (performance.now() >= deadline)
        throw new CanonicalMainError("canonical-main-timeout");
    return sha;
}

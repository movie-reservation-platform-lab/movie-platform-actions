...
import type { PublicationContext } from "./profile.mjs";
import { evaluateApprovedReport, renderApprovedSummary } from "./approved-evaluation.mjs";
import type { ApprovedDecision } from "./approved-evaluation.mjs";
...
type CapturedMember = { path: string; bytes: Buffer; maxBytes: number; label: string };
type CapturedEvidence = { provenance: CapturedMember; sbom: CapturedMember; report: CapturedMember };
 ...
 */
export async function generateV3Evidence(env: NodeJS.ProcessEnv, readPolicyJson?: PolicyReader, decisionTime?: () => string): Promise<void> {
  const context = readCandidateContext(env);
  const members = captureEvidenceMembers(context);
  const decision = await evaluateApprovedReport(
    members.report.bytes, context.image, context.profile.component, "immutable-ghcr",
    env.GH_TOKEN, readPolicyJson, decisionTime,
  );

  // Rejected runs still retain every finding and its explanation; these diagnostics are not canonical evidence.
  writeJson(
    join(context.workspace, "security-evidence/vulnerability-policy-diagnostic.json"),
    { diagnosticOnly: true, ...decision }, 1024 * 1024, "Policy diagnostic",
  );
  appendFileSync(context.summaryPath, renderApprovedSummary(decision, context.profile.artifact));
  requireRuntime(
    decision.evaluation.blockingCritical === 0,
    "Unapproved CRITICAL findings block canonical evidence; inspect the retained rejected diagnostics.",
  );
  ...
  writeJson(
    join(context.workspace, "security-evidence/component-candidate-evidence-v1alpha3.json"),
    candidate, 1024 * 1024, "V3 candidate",
  );
}

/** Validate the canonical producer/run identity and the image provenance reference before reading files. */
function readCandidateContext(env: NodeJS.ProcessEnv): CandidateContext {
...
function captureEvidenceMembers(context: CandidateContext): CapturedEvidence {
  const capture = (filename: string, maxBytes: number, label: string): CapturedMember => {
...
function requireUnchangedMembers(workspace: string, members: CapturedEvidence): void {
...
function createCandidateDocument(context: CandidateContext, members: CapturedEvidence, decision: ApprovedDecision, generatedAt: string) {

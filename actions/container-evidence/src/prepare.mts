...
type PreparationFailureCode = keyof typeof failureMessages;
...
export async function prepareCandidate(env: NodeJS.ProcessEnv): Promise<void> {
  let profile;
  try {
    profile = publicationContext(env);
    if (env.GITHUB_API_URL !== "https://api.github.com") throw new Error();
  } catch { throw new PreparationError("invalid-publication-context"); }
  ...
  if (canonicalSha !== env.GITHUB_SHA) throw new PreparationError("stale-main-revision");
  try {
    appendFileSync(outputPath, `image_ref=${profile.image}\ntag=${profile.tag}\nartifact=${profile.artifact}\n`);
  } catch { throw new PreparationError("prepare-output-failed"); }
}

/** Map known codes to controlled prose; never display arbitrary error text. */
export function preparationFailure(error: unknown): string {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  try { await prepareCandidate(process.env); }
  catch (error) {

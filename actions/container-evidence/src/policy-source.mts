...
import type { Exemption } from "./exemption-policy.mjs";
...
export type PolicySnapshot = { revision: string; records: Exemption[] };
/** The deadline is an absolute performance.now() value shared by every request in one acquisition. */
export type PolicyReader = (endpoint: string, maxBytes: number, deadlineMs: number) => Promise<unknown>;
type GitObjectReader = (objectPath: string, maxBytes?: number) => Promise<Record<string, unknown>>;
type GitTreeEntry = Record<string, unknown> & { path: string };

/** Create an authenticated reader with fixed HTTPS authority and no proxy/config or redirect fallback. */
export function githubPolicyReader(token: string | undefined): PolicyReader {
  requireRuntime(
    typeof token === "string" && /^[\x21-\x7e]{1,4096}$/.test(token),
    "Latest approval acquisition requires a GitHub token with read access to the actions repository.",
  );
  return async (endpoint, maxBytes, deadlineMs) => {
    const allowedObjectPath = /^(ref\/heads\/main|(commits|trees|blobs)\/[a-f0-9]{40})$/;
    requireRuntime(
      endpoint.startsWith(policyApiPrefix) && allowedObjectPath.test(endpoint.slice(policyApiPrefix.length)),
      "Unsupported policy endpoint.",
    );
...
function readAuthenticatedResponse(endpoint: string, token: string, maxBytes: number, deadlineMs: number): Promise<Buffer> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const failRequest = (error: RuntimeError) => {
      reject(error);
      httpRequest.destroy();
    };
    const timeout = setTimeout(
      () => failRequest(new RuntimeError("Approval acquisition exceeded its 120-second total limit.")),
      remainingMs,
    );
    httpRequest.on("error", (error) => reject(
...
async function readCompleteTree(readGitObject: GitObjectReader, treeSha: string): Promise<GitTreeEntry[]> {
  const treeResponse = await readGitObject(`trees/${treeSha}`);
  requireRuntime(
    treeResponse.sha === treeSha && treeResponse.truncated === false && Array.isArray(treeResponse.tree),
    "Approval tree identity is wrong or listing is incomplete.",
  );
  requireRuntime(
    treeResponse.tree.length <= maxTreeEntries,
    `Approval tree entry limit=4096, observed=${treeResponse.tree.length}; inspect policy locally.`,
  );
  const memberNames = new Set<string>();
  return treeResponse.tree.map((untrustedEntry: unknown) => {
    const entry = validateObject(untrustedEntry);
    const memberName = entry.path;
    requireRuntime(
      typeof memberName === "string" && memberName.length > 0 && memberName.length <= 255 &&
      memberName !== "." && memberName !== ".." &&
      !/[/\\\u0000-\u001f\u007f\ud800-\udfff]/.test(memberName) && !memberNames.has(memberName),
      "Approval tree contains ambiguous or unsafe members.",
    );
...
function findDirectorySha(entries: GitTreeEntry[], directoryName: string): string | undefined {
  const entry = entries.find((member) => member.path === directoryName);
  if (!entry) return undefined;
  requireRuntime(
    entry.type === "tree" && entry.mode === "040000",
    "Policy directory must be a Git tree, not a link or submodule.",
  );
...
function decodeVerifiedBlob(blob: Record<string, unknown>, expectedSha: string, expectedBytes: number): Buffer {
  requireRuntime(
    blob.sha === expectedSha && blob.size === expectedBytes && blob.encoding === "base64" && typeof blob.content === "string",
    "Approval blob metadata disagrees with its tree entry.",
  );
  const encodedContent = blob.content.replaceAll("\n", "");
  requireRuntime(
    /^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(encodedContent),
    "Approval blob encoding is invalid.",
  );
  ...
  requireRuntime(
    recordBytes.length === expectedBytes && recordBytes.toString("base64") === encodedContent && actualBlobSha === expectedSha,
    "Approval blob bytes disagree with their immutable identity.",
  );
  return recordBytes;
}

/** Read one regular EX-ID.json member and validate its filename, component and record semantics. */
async function readVerifiedApprovalRecord(component: string, entry: GitTreeEntry, readGitObject: GitObjectReader): Promise<Exemption> {
  requireRuntime(
    entry.type === "blob" && entry.mode === "100644" && /^EX-[A-Za-z0-9][A-Za-z0-9-]{0,60}\.json$/.test(entry.path),
    "Selected approvals must be regular non-executable EX-ID.json files.",
  );
  ...
  requireRuntime(
    expectedBytes > 0 && expectedBytes <= maxRawRecordBytes,
    `Approval ${component}/${entry.path} raw byte limit=65536, observed=${expectedBytes}; inspect policy locally.`,
  );
 ...
 */
export async function acquirePolicy(componentInput: string, readPolicyJson: PolicyReader): Promise<PolicySnapshot> {
  const component = componentName(componentInput);
  const deadlineMs = performance.now() + acquisitionTimeoutMs;
  const readGitObject: GitObjectReader = async (objectPath, maxBytes = maxMetadataBytes) =>
    validateObject(await readPolicyJson(policyApiPrefix + objectPath, maxBytes, deadlineMs));

  const mainReference = await readGitObject("ref/heads/main");
  const referenceTarget = validateObject(mainReference.object);
  requireRuntime(
    mainReference.ref === "refs/heads/main" && referenceTarget.type === "commit",
    "Policy source must resolve approved main to one commit.",
  );
  ...
  requireRuntime(
    selectedEntries.length <= maxSelectedRecords,
    `Approval record limit=128, observed=${selectedEntries.length}; inspect policy locally.`,
  );

  const loadedRecords: Exemption[] = [];
  ...
  let validatedRecords: Exemption[];
  try { validatedRecords = parseExemptions(loadedRecords, component); }
  catch (error) {
    if (error instanceof PolicyError) throw new RuntimeError(`Approval set for ${component} at ${revision} is invalid (${error.code}); inspect the central policy using the local scanning path.`);
    throw error;
  }

...
export type PolicyReader = (endpoint: string, limit: number, deadline: number) => Promise<unknown>;
    ...
    const bytes = await new Promise<Buffer>((resolve, reject) => {
      ...
      const req = request({
        ...
        response.on("data", (chunk: Buffer) => {
          size += chunk.length;
          if (size > limit) fail(new RuntimeError(`Approval response byte limit=${limit}, observed=${size}; inspect policy using the local scanning path.`));
          else chunks.push(chunk);
        });
        response.on("end", () => resolve(Buffer.concat(chunks)));
      });
      const fail = (error: RuntimeError) => { reject(error); req.destroy(); };
...
export async function acquirePolicy(componentInput: string, read: PolicyReader): Promise<PolicySnapshot> {
  const component = componentName(componentInput);
  const deadline = performance.now() + 120_000;
  const get = async (suffix: string, limit = metadataLimit) => validateObject(await read(prefix + suffix, limit, deadline));
  const tree = async (sha: string): Promise<Entry[]> => {
    ...
    return response.tree.map((item: unknown) => {
  ...
  const child = (entries: Entry[], name: string): string | undefined => {
  ...
  let validated: Exemption[];
  try { validated = parseExemptions(records, component); }
  catch (error) {

...
export type Finding = {
  ...
  package: { name: string; version: string; purl: string | null };
  severity: Severity;
};
 ...
 */
export function readTrivyFindings(untrustedReport: unknown, expectedImage: string, requirePlatform: boolean): Finding[] {
  ...
  for (const [resultIndex, untrustedResult] of results.entries()) {
    ...
    for (const [findingIndex, untrustedFinding] of vulnerabilities.entries()) {
      ...
      assertPolicyCondition(severities.includes(severity as Severity), "unsupported-severity");
 ...
 */
function eligiblePurl(packageIdentifier: unknown, installedVersion: string): string | null {
  if (packageIdentifier === undefined || packageIdentifier === null) return null;
  try {

 ...
 */
export function assertPolicyCondition(condition: unknown, errorCode: string): asserts condition {
  if (!condition) throw new PolicyError(errorCode);
}
 ...
 */
export function validateObject(untrustedObject: unknown, requiredKeys?: readonly string[]): Record<string, unknown> {
  assertPolicyCondition(typeof untrustedObject === "object" && untrustedObject !== null && !Array.isArray(untrustedObject), "invalid-object");
  const record = untrustedObject as Record<string, unknown>;
  if (requiredKeys) {
 ...
 */
export function validateText(untrustedText: unknown, maxCharacters = 2048): string {
  assertPolicyCondition(
    typeof untrustedText === "string" && untrustedText.length > 0 && untrustedText.length <= maxCharacters * 2 && [...untrustedText].length <= maxCharacters &&
    untrustedText === untrustedText.trim() && !/[\u0000-\u001f\u007f\ud800-\udfff]/u.test(untrustedText),
    "invalid-text",
  );
 ...
 */
export function validateArray(untrustedArray: unknown, maxItems: number): unknown[] {
 ...
 */
export function parseUtcTimestamp(untrustedTimestamp: unknown): number {
 ...
 */
export function validateReferenceUrl(untrustedUrl: unknown): string {
  const referenceUrl = validateText(untrustedUrl);
  let parsedUrl: URL;
  try { parsedUrl = new URL(referenceUrl); } catch { throw new PolicyError("invalid-reference"); }
  assertPolicyCondition(
    referenceUrl.startsWith("https://") && /^[\x21-\x7e]+$/.test(referenceUrl) &&
 ...
 */
export function validatePackagePurl(untrustedPurl: unknown, installedVersion: string): string {
  ...
  } catch { throw new PolicyError("invalid-purl"); }
